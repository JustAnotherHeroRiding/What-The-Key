...
function LibraryOrDeletedScreen({
  ...
  navigation: LibraryOrDeletedScreenNavigationProp;
}) {
  ...
  const { data: tracks, isFetching, error, refetch, isLoading } = useQuery({
    queryKey: [type], queryFn: () => getTracks({ location: type }),
    staleTime: 500,
      ...
      {isValidType ? (
        isLoading ? (
          <View style={tw.style(`flex items-center justify-center my-auto`)}>
            <Text style={tw.style(`text-white text-center`, { fontFamily: "figtree-bold" })}>Building your {TitleCaseMap[type]}...</Text>
            <LoadingSpinner />
          </View>
        ) : isApiErrorResponse(tracks) ? (
          <View>
            <Text style={tw.style(`text-white border-slate-500 border-b-2 font-figtreeBold text-3xl py-4 text-center`)}>{TitleCaseMap[type]}</Text>
            <Text style={tw.style(`text-white font-figtreeBold text-3xl py-4 text-center`)}>Your {TitleCaseMap[type]} is empty</Text>
            <TouchableOpacity
